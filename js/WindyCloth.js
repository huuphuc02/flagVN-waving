...
class Particle {
    ...
    addForce(force) {//Thêm một lực vào hạt bằng cách cộng dồn vào vector gia tốc a.
        this.a.add(
                this.tmp2.copy(force).multiplyScalar(this.invMass)
                );
    }
    integrate(timesq) {//Tích phân vị trí của hạt dựa trên vector gia tốc a, với timesq là bình phương của thời gian
...
class WindyCloth {
    ...
    constructor(params) {
        ...
        const w = params.xSegs || 50;
        const h = params.ySegs || 50;
        this.w = w;
        this.h = h;
        this.pinsTop = [];
        this.pinsRight = [];
        ...
        this.velocity = new THREE.Vector3(params.VELOCITY * 2, 0, 0).multiplyScalar(params.MASS);
    ...
    satisfyConstraints(p1, p2, distance) {//Giải quyết các ràng buộc giữa hai hạt.
    ...
    simulate(now, speed) {//Mô phỏng các hiệu ứng của lá cờ dựa trên thời gian now

        const windStrength = Math.cos(now / 7000) * 20 + 40;
        const adjustedWindStrength = windStrength * speed;
      //  console.log(speed);


        this.params.windForce.set(Math.sin(now / 2000), Math.cos(now / 3000), Math.sin(now / 1000));
        this.params.windForce.normalize();
        this.params.windForce.multiplyScalar(adjustedWindStrength);

        // Aerodynamics forces(lực đối lưu)
